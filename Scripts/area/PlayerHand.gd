...
func calculate_hand_size():
	if card_position_in_hand.size() > 0:
		hand_x_position_min = float(card_position_in_hand.min())
		hand_x_position_max = float(card_position_in_hand.max())
	else:
...
func update_hand_positions(card, index):
	...
	card_position_in_hand.append(x_position)
...
func get_drop_index(mouse_pos_x):
	for i in card_position_in_hand.size():
		if card_position_in_hand[i] > mouse_pos_x:
			return i
